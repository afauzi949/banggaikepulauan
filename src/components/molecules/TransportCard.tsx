...
export function TransportCard({ transport, className }: TransportCardProps) {
  const { type, operator, origin, destination, price, currency, schedule } =
    transport;

  return (
    <article
      className={cn(
        "flex w-full flex-col gap-3 rounded-2xl border border-[#e5e7eb] bg-white p-5",
        className,
      )}
    >
      ...
      <div className="flex flex-col gap-1">
        <h3 className="font-[family-name:var(--font-dm-sans)] text-[24px] font-bold leading-[32px] text-[#111827]">
          {origin} <span aria-hidden>➔</span>{" "}
          <span className="sr-only">menuju</span>
          {destination}
        </h3>
